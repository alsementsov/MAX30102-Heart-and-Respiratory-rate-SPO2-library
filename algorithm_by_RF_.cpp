...
void rf_heart_rate_and_oxygen_saturation(uint32_t *ir_buffer, int32_t n_ir_buffer_length, uint32_t *red_buffer, float *pn_spo2, int8_t *pch_spo2_valid, 
...
*/
{
  ...
  for (k=0; k<n_ir_buffer_length; ++k) {
    f_ir_mean += ir_buffer[k];
    f_red_mean += red_buffer[k];
  }
  f_ir_mean=f_ir_mean/n_ir_buffer_length ;/// Calculate average value
  f_red_mean=f_red_mean/n_ir_buffer_length ;/// Calculate average value
  
  // remove DC 
  for (k=0,ptr_x=an_x,ptr_y=an_y; k<n_ir_buffer_length; ++k,++ptr_x,++ptr_y) {
    *ptr_x = ir_buffer[k] - f_ir_mean;
    *ptr_y = red_buffer[k] - f_red_mean;
  }
